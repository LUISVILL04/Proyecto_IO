...
solucionIteracion = uic.loadUi("mostrarSolucionIteracion.ui")


def ingresarCoeficientes(): 
    primera.hide() #cerramos la ventana principal
    funcRestr.show()  # abrimos la ventana de los valores
...
def continuarSolucion():
    solucion.hide()
    solucionIteracion.show()    
    ...
    for i in range(len(A)):
        for j in range(len(A[0])):
            ...
            if A == 1 or A == 0:
                ...
                solucionIteracion.espacioMatrizB.setText(str(B))
                
    print(B)
    print(A)

def botones():
    primera.boton1.clicked.connect(ingresarCoeficientes)  # cuando presiono el boton maximizar (boton prueba) hace el llamdado a la funcion ventana1
    funcRestr.Btn_continuar.clicked.connect(BtnContinuar)
    ...
    solucion.Btn_continuar.clicked.connect(continuarSolucion)
    solucion.Btn_continuar.clicked.connect(solucionesIteraciones)
